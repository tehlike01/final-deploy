    ...
    for ticket_data in tickets_data:
        ...
        if str(ticket_data['name']) in existing_ticket_names:
            ...
            if ticket_data['tags'] == 'New' and existing_ticket['stage_id'][0] != 1: 
                changes['stage_id'] = 1
            elif ticket_data['tags'] == 'Resolved' and existing_ticket['stage_id'][0] != 6:  
                changes['stage_id'] = 6
        ...
        if tickets:
            ...
            for comment_id in comments_to_post:
                ...
                if comment_response.status_code == 200:
                    ...
                    if comment_response.status_code == 422 and "Body can't be blank" in comment_response.text:
                        log("No new comments to post.")
                        break
                    else:
...
#----------------------UPDATING COMMENT IS API------------------------------
def update_ticket_in_api(ticket_number):
    ticket_api = get_ticket_from_api.get_ticket_by_number(ticket_number)
    ticket_id = get_id_by_ticket_name(ticket_api['name'])
    ticket_database = models.execute_kw(db, uid, password, 'help.ticket', 'read', [[ticket_id]])[0]

    changes = {}

    if ticket_database['stage_id'][0] == 1 and ticket_api['tags'] != 'New': 
        changes['status'] = 'New'
    elif ticket_database['stage_id'][0] == 3 and ticket_api['tags'] != 'In Progress':  
        changes['status'] = 'In Progress'
    elif ticket_database['stage_id'][0] == 6 and ticket_api['tags'] != 'Resolved':
        changes['status'] = 'Resolved'

    if ticket_database['subject'] != ticket_api['subject']:
        changes['subject'] = ticket_database['subject']
    if ticket_database['customer_name'] != ticket_api['customer_name']:
        changes['customer_name'] = ticket_database['customer_name']
    if changes:
        base_url = "https://itmsc.syncromsp.com/api/v1/tickets"
        headers = {
            "accept": "application/json",
        ...
        if response.status_code == 200:
            ...
            if tickets:
                ticket_id = tickets[0].get("id")
                update_url = f"{base_url}/{ticket_id}"
                response = requests.put(update_url, headers=headers, json=changes)
                if response.status_code == 200:
                    log("Ticket {} updated successfully.".format(ticket_number))
                else:
                    log(f"Failed to update ticket. Status code: {response.status_code}")
            else:
                log("No ticket found with the provided number.")
        else:
            log(f"Failed to retrieve ticket data. Status code: {response.status_code}")
    else:
        log("No changes detected for {}.".format(ticket_number))

def update_all_tickets_in_api(customer_id):
    ticket_numbers = get_ticket_from_api.get_all_ticket_numbers(customer_id)
    for ticket_number in ticket_numbers:
        update_ticket_in_api(ticket_number)
